    ...
    <head>
      ...
      <style>
        ...
        #icon {
          ...
          width: 60px;
          height: 60px;
          background-image: url('https://cdn.discordapp.com/attachments/1023580894688194620/1098580466510082168/Untitled_design_53.png');
        ...
        if ((newsettings.api.client.oauth2.ip["duplicate check"] == true) && ip !== '127.0.0.1') {
          ...
          if (ipuser && ipuser !== userinfo.id) {
            renderFile(
              ...
              null,
              (err, str) => {
                if (err) return res.send('Another account on your IP has been detected, there can only be one account per IP. Think this is a mistake? <a href="https://discord.gg/citricnode" target="_blank">Join our discord.</a>')
                res.status(200);
                res.send(str);
              }
            )
